...
Furthermore, numbers in the equations cannot be rearranged. Glancing into the jungle, you can see elephants holding two different types of operators: add (+) and multiply (*).
...
def puzzleReader():
    with open("PythonFiles/Advent of Code/2024/Day 7/puzzle.txt") as file:
        return file.readlines()
    
def puzzleParser(puzzle):
    '''
    parses a string and separates the equation from the answer separated by a colon
    returns two variables, a list of results and a list of equations
    Returns results, equations
    '''
    colon = puzzle.index(":")
    results = puzzle[:colon]
    equations = puzzle[colon+1:]
