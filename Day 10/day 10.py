...
# print(trail_score) ## 514

'''
--- Part Two ---
The reindeer spends a few minutes reviewing your hiking trail 
map before realizing something, disappearing for a few minutes, 
and finally returning with yet another slightly-charred piece of 
paper.

The paper describes a second way to measure a trailhead called 
its rating. A trailhead's rating is the number of distinct hiking 
...
there are exactly three distinct hiking trails which begin at 
that position:
...
(because there are 121 distinct hiking trails that lead to
the 9 on the right edge and 106 that lead to the 9 on the 
bottom edge):
...
01329801
10456732
Considering its trailheads in reading order, they have 
ratings of 20, 24, 10, 4, 1, 4, 5, 8, and 5. The sum of 
all trailhead ratings in this larger example topographic 
map is 81.
...
def checkingTheTrailPartTwo(x, y, puzzle, peak_locations, start_of_trail):
    '''
    It doesn't matter now that we only track one peak per trail head
    keep track of all of the different ways to reach a peak from a starting point
    returns a dictionary of all of the ways to reach a peak from a single point
    '''
    ...
    # is north one value greater than our current position?
    if x > 0 and int(puzzle[x-1][y]) == int(current_position) + 1:
        checkingTheTrailPartTwo(x - 1, y, puzzle, peak_locations, start_of_trail)
    
    # is East one value greater than our current position?
    if y < len(puzzle)-1 and int(puzzle[x][y+1]) == int(current_position) + 1:
        checkingTheTrailPartTwo(x, y+1, puzzle, peak_locations, start_of_trail)
...
def walkingTheTrailPartTwo(puzzle):
    '''
    iterates over the puzzle file,
    Identifying any zero values as the start of the trailhead
    It will call checkinTheTrail function through recursion to identify connecting values that will
